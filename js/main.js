...
const AppController = {
    ...
    setupQRScreenHandlers: function() {
        ...
        const backButton = document.querySelector('#qrcode-screen .back-button');
        if (backButton) {
            backButton.addEventListener('click', () => {
    ...
    updateTrainStats: function(trains) {
        ...
        // Вывод статистики по типам поездов
        for (const [type, count] of Object.entries(this.countTrainsByType(trains))) {
            statsContainer.innerHTML += `${type} - ${count}<br>`;
        }
        
        // Обновление общего количества
        document.querySelector('.stats-title').textContent = `Доступно составов: ${trains.length}`;
    },
